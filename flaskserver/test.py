...
'''def process_image(image):
    try:
        ...
        for im in results.ims:
            buffered = BytesIO()
            im_base64 = Image.fromarray(im)
            image_proceed=im_base64.save(buffered, format="JPEG")
            #base64_images.append(base64.b64encode(buffered.getvalue()).decode('utf-8'))
...
def upload_image():
    ...
    processing_result = process_image(image)
    image.save(os.path.join('proceed', processing_result))
    last_uploaded_image = processing_result
    return 'Image saved on server', 200 '''
...
def upload_image():
    ...
    # Process the image and get the result
    processing_result = process_image(os.path.join('uploads', new_filename))
